...
VOID InsertTreeNode( struct NListtree_Data *data, struct MUI_NListtree_ListNode *ln, struct MUI_NListtree_TreeNode *tn1, struct MUI_NListtree_TreeNode *tn2 )
{
  ...
  */
  if ( (IPTR)tn2 == INSERT_POS_HEAD )
  {
    AddHead( (struct List *)&ln->ln_List, (struct Node *)&tn1->tn_Node );
  }
  else if ( (IPTR)tn2 == INSERT_POS_TAIL )
  {
  ...
  if ( ln && tn )
  {
    ...
    if ( tn )
    {
      if ( ( ( tn->tn_Flags & TNF_LIST ) && !( tn->tn_Flags & TNF_OPEN ) ) || openall )
      {
        ...
        if ((IPTR)msg->ListNode == MUIV_NListtree_Open_ListNode_Parent)
        {
          tn2 = tn;
...
IPTR _NListtree_Rename(struct IClass *cl, Object *obj, struct MUIP_NListtree_Rename *msg)
{
  ...
  */
  if ( (IPTR)msg->TreeNode == MUIV_NListtree_Rename_TreeNode_Active )
    tn = data->ActiveNode;
  else
    tn = msg->TreeNode;
...
IPTR _NListtree_GetNr(struct IClass *cl, Object *obj, struct MUIP_NListtree_GetNr *msg)
{
  ...
  **  Handle special events.
  */
  if ( (IPTR)msg->TreeNode == MUIV_NListtree_GetNr_TreeNode_Active )
    tn = data->ActiveNode;
  else if ( (IPTR)msg->TreeNode == MUIV_NListtree_GetNr_TreeNode_Root )
    tn = CTN((APTR)&data->RootList);
  else
    tn = msg->TreeNode;
...
IPTR _NListtree_Redraw(struct IClass *cl, Object *obj, struct MUIP_NListtree_Redraw *msg)
{
  ...
  if(((IPTR)msg->TreeNode != MUIV_NListtree_Redraw_Active) && ((IPTR)msg->TreeNode != MUIV_NListtree_Redraw_All) && !(msg->Flags & MUIV_NListtree_Redraw_Flag_Nr))
  {
    LONG pos;
...
IPTR _NListtree_NextSelected(struct IClass *cl, Object *obj, struct MUIP_NListtree_NextSelected *msg)
{
  ...
  if ( (IPTR)*msg->TreeNode == MUIV_NListtree_NextSelected_Start )
    curr = 0;
  else
    curr++;
  ...
  if ( ( curr > -1 ) && data->SelectedTable.tb_Entries )
  {
    ...
    else if(((IPTR)*msg->TreeNode == MUIV_NListtree_NextSelected_Start) &&
             (data->ActiveNode != MUIV_NListtree_Active_Off))
    {
...
IPTR _NListtree_PrevSelected(struct IClass *cl, Object *obj, struct MUIP_NListtree_PrevSelected *msg)
{
  ...
  if((IPTR)*msg->TreeNode == MUIV_NListtree_PrevSelected_Start)
